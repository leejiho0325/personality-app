...
const catColor = { "내면 심리": "#b8976a", "관계와 애착": "#6a8ab8", "자아상": "#8a6ab8", "삶의 태도": "#6ab88a", "연애": "#c46a8a", "핵심": "#c0bdb8" };
...
function Intro({ onStart }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"100vh", padding:"2rem 1.4rem", textAlign:"center" }}>
      ...
      <p style={{ fontFamily:"monospace", fontSize:"0.58rem", color:C.border, letterSpacing:"0.1em", margin:"1.6rem 0 2rem" }}>
        13개 질문 · 솔직할수록 정확합니다
      </p>
...
export default function PersonalityApp() {
  ...
  const handleFinish = async (answers) => {
    ...
    try {
      ...
      setResult({ ...JSON.parse(t1), ...JSON.parse(t2) });
      setPhase("result");
    } catch (e) {
